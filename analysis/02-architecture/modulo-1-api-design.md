...
| `GET` | `/asientos` | Lista todos los asientos con sus líneas | — |
| `GET` | `/balance` | Saldo acumulado por cuenta, a la fecha actual (sin filtro de periodo todavía) | — |

## 4. Cálculo del balance

Cada cuenta tiene una "naturaleza": para Activo y Gasto, el saldo normal es `débito − crédito`; para Pasivo, Patrimonio e Ingreso, es `crédito − débito`. El endpoint `/balance` sigue esta regla por cada cuenta usando todas las líneas de asiento existentes. No hay filtro por fecha en este módulo — es el saldo acumulado histórico completo (queda para el Módulo 4 el reporte con corte por periodo).
